...
def main():
    for popper in range(24, 40):
        np.random.seed(123)
        holding = []
        file = open(r'shipsnet.json')
        dataset = json.load(file)
        dataset['data'].pop(popper)
        dataset['labels'].pop(popper)
        for i in range(100):
            # Load Dataset
            images = []
            for index in range(len(dataset['data'])):
                ...
                ims = resize(im, (120, 120), mode='constant')
                images.append(ims)
            ...
            answer = evaluate(model, batch, num_epochs, verb, x_train, x_test, y_train, y_test)
            print(answer)
            holding.append(answer)
            print(len(holding))
        print(holding)
        print("Next Iteration")
        f = open("answers_local.txt", "a+")
        f.write("This is for popped image at index " + str(popper) + ": " + str(holding) + '\n')
        f.close()
